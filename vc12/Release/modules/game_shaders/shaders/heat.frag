...
float col(vec2 coord)
{
    ...
    for(int i = 0; i  steps; i++) {
        vec2 adjc = coord;
        theta = delta_thetafloat(i);
        adjc.x += cos(theta)u_Timespeed + u_Time  speed_x;
        adjc.y -= sin(theta)u_Timespeed - u_Time  speed_y;
        col = col + cos( (adjc.xcos(theta) - adjc.ysin(theta))frequency)intensity;
    }
    return cos(col);
}
...
void main(void)
{
    ...
    c2.x += u_Resolution.xdelta;
    float dx = emboss(cc1-col(c2))delta;

    c2.x = p.x;
    c2.y += u_Resolution.ydelta;
